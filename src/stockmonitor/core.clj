...
(def stock-url "https://query1.finance.yahoo.com/v7/finance/quote?symbols=%s")
(def stock-history-url "https://query1.finance.yahoo.com/v7/finance/download/%s?period1=%s&period2=%s&interval=1d&events=history&includeAdjustedClose=true")
(def interesting-stock-symbols #{"amz.de"})
...
(defn get-stock-history
  [filepath]
  (comment
    (get-stock-history "https://query1.finance.yahoo.com/v7/finance/download/AMZ.DE?period1=1603238400&period2=1603670400&interval=1d&events=history&includeAdjustedClose=true")
    ({:Date     "2020-10-21",
      :Open     "2733.000000",
      :High     "2733.000000",
      :Low      "2664.500000",
...
(defn get-stock-history-from-url
  [stock-symbol start-timestamp end-timestamp]
  (-> stock-history-url
      (format stock-symbol start-timestamp end-timestamp)
      get-stock-history))

(defn remove-unused-fields
  [stock]
  (dissoc stock :InsertSymbolHere))

(defn get-stock-history-with-interesting-fields
  [filename]
  (->> filename
       get-stock-history
...
(defn get-stock
  [filename]
  (comment
    (get-stock "https://query1.finance.yahoo.com/v7/finance/quote?symbols=amz.de")
